...
export function IceCreamFlavors() {
  return (
    <FadeInSection id="sabores" className="bg-surface py-24">
      <div className="mx-auto max-w-7xl px-6">
          ...
          <div>
            ...
            <FadeInBlock delay={0.12}>
              <p className="mt-4 max-w-xl leading-relaxed text-on-surface/75">
                Sin fotos de postureo: aquí va lo que de verdad entra en la
                tarrina.
              </p>
            </FadeInBlock>
          </div>
        ...
        >
          {FLAVORS_SHOWCASE.map((item, i) => (
            <motion.article
              key={item.name}
              variants={staggerItem}
              className={`flex flex-col rounded-[2rem] border border-primary/15 bg-surface-container-lowest p-6 shadow-ambient transition duration-200 ease-out hover:-translate-y-0.5 hover:shadow-ambient-hover ${
            ...
            >
              ...
              <ul className="mt-5 flex flex-wrap gap-2">
                {item.ingredients.map((ing) => (
                  <li
                    key={ing}
                    className="rounded-full bg-primary/10 px-3 py-1.5 text-xs font-medium text-on-surface"
                  >
                    {ing}
                  </li>
                ))}
